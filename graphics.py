...
def render_stat_highlight_card(kind: str, topic: str, stat_pairs: list[tuple[str, str]],
    ...
    label_font = _font(24, "semibold")
    for i, (value, label) in enumerate(stat_pairs):
        cx = col_w * i + col_w / 2
        vw = _text_w(draw, str(value), value_font)
        draw.text((cx - vw / 2, y), str(value), font=value_font, fill=accent)
        lw = _text_w(draw, label.upper(), label_font)
        draw.text((cx - lw / 2, y + 84), label.upper(), font=label_font, fill="#E7ECFA")
    y += 84 + 40 + 40
    ...
    if hook:
        ...
        for line in hook_lines:
            ...
            draw.text((W / 2 - hw / 2, y), line, font=hook_font, fill="#C7D0E8")
            y += 40
...
def render_comparison_card(kind: str, a_name: str, b_name: str,
    ...
    vs_font = _font(32, "extrabold")
    vw = _text_w(draw, "VS", vs_font)
    draw.text((W / 2 - vw / 2, y + 6), "VS", font=vs_font, fill=accent)
    ...
    if a_team or b_team:
        ...
        if a_team:
            ...
            draw.text((W * 0.27 - tw / 2, y), a_team, font=team_font, fill="#93A3D1")
        if b_team:
            tw = _text_w(draw, b_team, team_font)
            draw.text((W * 0.73 - tw / 2, y), b_team, font=team_font, fill="#93A3D1")
        y += 44

    y += 20
    label_font = _font(22, "semibold")
    row_h = 96
    ...
    for label, a_val, b_val in stat_rows:
        ...
        lw = _text_w(draw, label.upper(), label_font)
        draw.text((W / 2 - lw / 2, y + 16), label.upper(), font=label_font, fill="#93A3D1")
        y += row_h
