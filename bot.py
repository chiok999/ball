...
def maybe_post_filler(matches: list):
    ...
    at most once every FILLER_INTERVAL_MINUTES, and only when:
      - filler content is enabled at all, and
      - no match is actively live right now (never compete with real
        coverage — a goal or halftime post always matters more), and
      - we're either within FILLER_PRE_MATCH_HOURS of a scheduled kickoff
        today, or inside the fixed local FILLER_WINDOW_START_HOUR-
        FILLER_WINDOW_END_HOUR window (covers no-match days, or match-day
        hours before the pre-match window opens)."""
    ...
    local_hour = int((datetime.now(timezone.utc).hour + config.FILLER_TZ_OFFSET_HOURS) % 24)
    in_fixed_window = config.FILLER_WINDOW_START_HOUR <= local_hour < config.FILLER_WINDOW_END_HOUR

    in_pre_match_window = False
    for m in matches:
        if m["status"] != "SCHEDULED":
            continue
        mins = _minutes_until_kickoff(m)
        if mins is not None and 0 <= mins <= config.FILLER_PRE_MATCH_HOURS * 60:
            in_pre_match_window = True
            break

    if not (in_fixed_window or in_pre_match_window):
        return
