...
# Transfermarkt hourly highlight cadence — independent clock.
_last_transfermarkt_post_time: float = 0.0
...
# ══════════════════════════════════════════════════════════════════
# TRANSFER NEWS — immediate, not tied to the filler clock
# ══════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════
# TRANSFERMARKT WEEKLY-DATASET HIGHLIGHT — one per hour, real data only
# ══════════════════════════════════════════════════════════════════

def maybe_post_transfermarkt_highlight():
    global _last_transfermarkt_post_time
    if not config.POST_TRANSFERMARKT_HIGHLIGHTS:
        return
    elapsed_min = (time.time() - _last_transfermarkt_post_time) / 60
    if elapsed_min < config.TRANSFERMARKT_INTERVAL_MIN:
        return

    try:
        item = transfermarkt.get_next_highlight(set(_events.keys()))
    except Exception as e:
        print(f"[TRANSFERMARKT] ⚠️  {e}")
        return

    if not item:
        # Nothing new this cycle (cache stale/empty/exhausted) — try again
        # next hour rather than spamming retries every poll.
        _last_transfermarkt_post_time = time.time()
        return

    print(f"[TRANSFERMARKT] 🔄 {item['player']}: {item['from_club']} → {item['to_club']}")
...
def main():
    global _last_event_post_time, _last_transfermarkt_post_time
    _load_state()
    _last_event_post_time = time.time()
    _last_transfermarkt_post_time = time.time()
    _start_keepalive()
    ...
    print(f"  Transfer news : {config.POST_TRANSFER_NEWS} ({', '.join(config.TRANSFER_LEAGUES.values())})")
    ...
    while True:
        try:
            ...
            maybe_post_transfermarkt_highlight()
