...
def _post_now(message: str) -> bool:
...
def maybe_post_filler(matches: list):
    ...
    if kind == "top_scorers" and config.WORLD_CUP_MODE:
        ...
        if scorers:
            ...
            if _post_now(poster.fmt_top_scorers(scorers)):
                return
    ...
    if config.POST_WIN_PROBABILITY:
        ...
        if upcoming:
            ...
            if probs:
                ...
                _post_now(poster.fmt_win_probability(m, probs))
...
def maybe_post_transfer_news(tick: int):
    ...
    for item in items:
        ...
        _post_if_new(item["key"], poster.fmt_transfer_news(item))
        time.sleep(2)
...
def maybe_post_stats(matches: list):
    ...
    leagues = stats_module.todays_leagues()  # 0, 1, or 2 leagues depending on retirements
    ...
    for slot, hour in sched.items():
        ...
        _run_stats_slot(slot, leagues)
        _stats_posted.add(slot)
        _save_state()
        time.sleep(3)  # small gap between consecutive stats posts


def _run_stats_slot(slot: str, leagues: tuple):
    """Fetch data and post for a single stats slot."""
    if slot == "standings_1" and len(leagues) >= 1:
        rows = stats_module.get_standings(leagues[0])
        if rows:
            poster.post(poster.fmt_standings(leagues[0], rows))
        else:
            print(f"[STATS] ⚠️  No standings data for slot {slot}")

    elif slot == "standings_2" and len(leagues) >= 2:
        rows = stats_module.get_standings(leagues[1])
        if rows:
            poster.post(poster.fmt_standings(leagues[1], rows))
        else:
            print(f"[STATS] ⚠️  No standings data for slot {slot}")
    ...
    if config.POST_VAR_DISALLOWED:
        for v in match.get("var_events", []):
            ...
            if not _already_posted(key):
                ...
                _post_if_new(key, poster.fmt_var_disallowed(match, v))
