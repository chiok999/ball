...
_filler_used:       dict[str, str]   = {}    # content_id -> iso timestamp, no-repeat tracking for filler.py
_last_filler_post_time: float        = 0.0   # separate cadence gate, once per FILLER_INTERVAL_MINUTES

# World Cup filler cadence — any real match event resets this clock.


def _load_state():
    global _events, _last_preview_date, _filler_used, _last_filler_post_time
    ...
    try:
        ...
        _filler_used            = raw.get("filler_used", {})
        _last_filler_post_time  = raw.get("last_filler_post_time", 0.0)
    ...
    try:
        with open(STATE_FILE, "w") as f:
            json.dump({
                ...
                "filler_used":           _filler_used,
                "last_filler_post_time": _last_filler_post_time,
            }, f)
    except Exception as e:
...
def _key_goal(mid: str, g: dict, idx: int = 0) -> str:
    # Prefer ESPN's own stable play id — immune to the clock
    # displayValue shifting between polls (e.g. "45:00" -> "45+2"),
    # which used to mint a new key and repost the same goal. Only
    # falls back to the old scorer+minute scheme if ESPN didn't supply
    # an id for this play at all.
...
# ══════════════════════════════════════════════════════════════════
# FOOTBALL NEWS — deal done, deal collapsed, manager sackings/
# transfers. Immediate, not tied to any clock. Present/future only —
# transfers.py enforces freshness, this never replays old news.
# ══════════════════════════════════════════════════════════════════
...
def maybe_post_transfer_news(tick: int):
    ...
    if not (config.TRANSFER_NEWS_START_HOUR <= now_hour < config.TRANSFER_NEWS_END_HOUR):
        return
    try:
    ...
    for item in items:
        ...
        # TRANSFER_NEWS_CATEGORIES is the master allowlist (e.g. set to
        # just "deal_done" to quiet the page down to confirmed signings
        # only). POST_PLAYER_QUOTES / POST_INJURY_NEWS are additional,
        # more specific switches on top of it — a category must clear
        # both to post.
        if category not in getattr(config, "TRANSFER_NEWS_CATEGORIES", {category}):
            continue
...
def maybe_post_filler(matches: list):
    """Posts a player-stat spotlight (legend fact or live top-scorers card)
    at most once every FILLER_INTERVAL_MINUTES. Never fires while a match
    is actually live. Otherwise follows one of two schedules:

    MATCH DAY (today has at least one match, in local time):
      - hourly starting at FILLER_MATCHDAY_START_HOUR, stopping
        FILLER_PRE_MATCH_STOP_HOURS before the earliest kickoff today
        (that final stretch is handed off to lineup/pre-match hype
        content instead)
      - if every match today has finished with time still left in the
        day, resumes hourly (if FILLER_RESUME_AFTER_MATCH) through the
        end of FILLER_WINDOW_END_HOUR

    NO MATCH DAY (nothing scheduled today at all, local time):
      - hourly for the entire FILLER_WINDOW_START_HOUR-
        FILLER_WINDOW_END_HOUR window
    """
    global _last_filler_post_time
    if not config.POST_FILLER_CONTENT:
        return

    now = time.time()
    if now - _last_filler_post_time < config.FILLER_INTERVAL_MINUTES * 60:
        return

    if any(m["status"] in ("IN_PLAY", "PAUSED", "EXTRA_TIME", "SHOOTOUT") for m in matches):
        return  # live coverage always wins — never post filler over it

    tz_delta = timedelta(hours=config.FILLER_TZ_OFFSET_HOURS)
    local_now = datetime.now(timezone.utc) + tz_delta
    local_hour = local_now.hour
    local_today = local_now.date()

    def _local_kickoff_date(m):
        utc_str = m.get("utcDate", "")
        if not utc_str:
            return None
        try:
            ko = datetime.fromisoformat(utc_str.replace("Z", "+00:00"))
            return (ko + tz_delta).date()
        except Exception:
            return None

    todays_matches = [m for m in matches if _local_kickoff_date(m) == local_today]

    if not todays_matches:
        # NO MATCH DAY — hourly for the whole fixed window.
        window_active = config.FILLER_WINDOW_START_HOUR <= local_hour < config.FILLER_WINDOW_END_HOUR
    else:
        upcoming = [m for m in todays_matches if m["status"] == "SCHEDULED"]
        earliest_mins = min(
            (mins for m in upcoming if (mins := _minutes_until_kickoff(m)) is not None and mins > 0),
            default=None,
        )
        if earliest_mins is not None and earliest_mins > config.FILLER_PRE_MATCH_STOP_HOURS * 60:
            # MATCH DAY, pre-match phase — hourly from the matchday start hour
            # until the pre-match stop cutoff.
            window_active = local_hour >= config.FILLER_MATCHDAY_START_HOUR
        elif earliest_mins is not None:
            # Inside the final pre-match stretch — hand off to lineup/hype
            # content, no filler here.
            window_active = False
        else:
            # No upcoming match left today (all finished, or none were ever
            # "upcoming") — resume for the rest of the evening window if
            # configured to.
            window_active = config.FILLER_RESUME_AFTER_MATCH and \
                config.FILLER_WINDOW_START_HOUR <= local_hour < config.FILLER_WINDOW_END_HOUR

    if not window_active:
        return

    result = filler.build_filler_post(_filler_used)
    if not result:
        return  # everything on cooldown, or no pool/API key available
    caption, image_path = result

    key = f"filler:{uuid.uuid4().hex[:12]}"  # filler posts are always "new" content by design, not deduped against a stable match/news key — this only needs to be unique, not stable, so a timestamp's second-granularity isn't reliable enough
    if _post_if_new(key, caption, image_path=image_path):
        _last_filler_post_time = now
        _save_state()
        print("[BOT] 📊 Posted filler content")
...
def process_match(match: dict):
    ...
    # ── Goals (scorer + assist, when ESPN provides one) ─────────────
    ...
    # data (ESPN carries no player ratings; see scraper.get_man_of_the_match).
...
def main():
    ...
    print(f"  Football news : {config.POST_TRANSFER_NEWS} — deal-done, deal-collapsed, manager sackings/"
          f"transfers, injuries ({', '.join(config.TRANSFER_LEAGUES.values())}) "
