...
ASSISTS:
  Both scoringPlays and details[] can carry an athletesInvolved[] list.
  When present, athletesInvolved[0] is the scorer and athletesInvolved[1]
  (if present) is the assist provider. Defensively parsed — if the shape
  doesn't match, the goal is still posted, just without an assist.
"""
...
def _scorer_and_assist(athletes: list, fallback_scorer: str = "Unknown") -> tuple[str, str | None]:
    """
    Given an athletesInvolved[] list from ESPN (scoringPlays or details),
    returns (scorer_name, assist_name_or_None).
    athletesInvolved[0] is the scorer, athletesInvolved[1] (if present)
    is the assist provider. Defensive — any missing/odd shape just
    yields no assist rather than a bad guess.
    """
    if not athletes:
        return fallback_scorer, None
    scorer_name = athletes[0].get("displayName") or athletes[0].get("shortName") or fallback_scorer
    assist_name = None
    if len(athletes) > 1:
        assist_name = athletes[1].get("displayName") or athletes[1].get("shortName") or None
...
def _normalize_espn(event: dict, slug: str, league_name: str) -> dict | None:
    try:
        ...
        # ── Goals (with assist, when ESPN provides athletesInvolved) ──
        ...
        for play in comp.get("scoringPlays", []) or []:
            ...
            scorer_name, assist_name = _scorer_and_assist(athletes, play.get("text", "Unknown"))
            goals.append({
                "minute": play.get("clock", {}).get("displayValue", "?"),
                # ESPN's own stable play id — the reliable half of goal
                # dedup. The clock displayValue this key used to rely
                # on can shift between polls as stoppage time gets
                # finalized ("45:00" -> "45+2"), which used to silently
                # mint a new dedup key and repost the same goal.
                "_play_id": play.get("id"),
        ...
        if not goals:
            for detail in comp.get("details", []):
                ...
                if "goal" in dtype or "penalty" in dtype:
                    ...
                    scorer_name, assist_name = _scorer_and_assist(athletes)
                    goals.append({
                        "minute": detail.get("clock", {}).get("displayValue", "?"),
                        "_play_id": detail.get("id"),
                        "scorer": {"name": scorer_name},
                        "assist": {"name": assist_name} if assist_name else {},
                        "team":   {"shortName": home_name if is_home else away_name},
                        "isHome": is_home,
        ...
        var_events = _extract_var_events(comp, home_id, home_name, away_name)
        ...
        return {
            ...
            "goals":    goals,
            "bookings": bookings,
            "lineups":  [],   # populated separately via get_lineup() close to kickoff
        }
...
# ══════════════════════════════════════════════════════════════════
# LINEUPS — fixes the previous dead stub
# ══════════════════════════════════════════════════════════════════
# The /scoreboard endpoint never carries lineups (hence the old
# hardcoded `"lineups": []`). The /summary?event={id} endpoint does,
# usually populating ~60 min before kickoff. Still plain JSON/requests,
...
def get_man_of_the_match(match: dict) -> dict | None:
    """
    Man of the Match lookup. ESPN's free site API doesn't expose
    per-player ratings, so this always returns None today (caller
    skips the MOTM post rather than guessing). Kept as a stable hook
    for a future source that does carry ratings.
    """
    return None


def _extract_starters(players: list) -> list:
    """Pulls the starting XI out of a roster's player list. ESPN
    usually marks starters with a boolean `starter` field, but some
    competitions (lower-profile friendlies/cups) instead carry a
    `starter` string ("true"/"false") or omit it and rely on
    `position.order`/slot number to imply starters (first 11 by
    order). Tries the boolean field first, then falls back to
    treating the first 11 outfield entries as the XI so lineups still
    populate for feeds that shape this data slightly differently."""
    ...
    if not starters and players:
        # Fallback: no explicit starter flags found at all on this feed
        # shape — assume roster order lists starters first (common ESPN
        # convention) and take the first 11.
        starters = players[:11]
    return [
        {"player": {"name": p.get("athlete", {}).get("displayName", "?")}}
        for p in starters
    ]
...
def get_lineup(league_slug: str, event_id: str) -> list[dict]:
    ...
    yet published (normal before ~60 min pre-kickoff) or on any error.
    May return a list with just ONE team's lineup if only one side has
    released theirs yet — caller (poster.fmt_lineup) handles that by
    marking the other side "Pending..." rather than dropping it."""
    raw_id = event_id.replace("espn_", "") if event_id.startswith("espn_") else event_id
    url = f"{ESPN_SUMMARY_API}/{league_slug}/summary?event={raw_id}"
    data = _espn_get(url)
    if not data:
        print(f"[ESPN] Lineup fetch: no response for event {raw_id} ({league_slug})")
        return []
    try:
        # Primary location. Some competitions instead (or additionally)
        # carry roster data under boxscore.players — check both so a
        # feed-shape difference doesn't just silently return [].
        rosters = data.get("rosters", [])
        if not rosters:
            rosters = data.get("boxscore", {}).get("players", [])
        if not rosters:
            print(f"[ESPN] Lineup: no roster data yet for event {raw_id} (normal pre-~60min)")
            return []

        boxscore_form = data.get("boxscore", {}).get("form", [])
        formations = data.get("boxscore", {}).get("formations", [])
        lineups = []
        for i, roster in enumerate(rosters[:2]):
            players = roster.get("roster", []) or roster.get("statistics", [])
            if not players:
                continue
            team_name = (
                boxscore_form[i]["team"]["displayName"]
                if i < len(boxscore_form) and "team" in boxscore_form[i]
                else roster.get("team", {}).get("displayName", "?")
            )
            starters = _extract_starters(players)
            if starters:
                lineups.append({
                    "team":      team_name,
                    "formation": formations[i].get("displayName", "") if i < len(formations) else "",
                    "startXI":   starters,
                })
        if not lineups:
            print(f"[ESPN] Lineup: roster data present but no starters extracted for event {raw_id}")
        return lineups
    except Exception as e:
        print(f"[ESPN] Lineup parse error for event {raw_id}: {e}")
        return []


# ══════════════════════════════════════════════════════════════════
# VAR / DISALLOWED GOAL DETECTION (best-effort)
# ══════════════════════════════════════════════════════════════════
# ESPN doesn't have a clean "disallowed goal" field. This scans the
# details[] text for VAR-review language. Deliberately conservative:
# if it can't confidently confirm a disallowed goal, it returns nothing
# rather than risk posting a false "No Goal" — this needs live
# verification against a real match, since it can't be tested offline.

_VAR_DISALLOWED_KEYWORDS = (
    "disallowed", "goal disallowed", "no goal", "ruled out",
    "overturned", "var review - goal", "goal cancelled", "goal canceled",
)


def _extract_var_events(comp: dict, home_id: str, home_name: str, away_name: str) -> list[dict]:
    var_events = []
    for detail in comp.get("details", []):
        dtype = (detail.get("type", {}).get("text", "") or "").lower()
        if not any(kw in dtype for kw in _VAR_DISALLOWED_KEYWORDS):
            continue
        is_home  = detail.get("team", {}).get("id", "") == home_id
        athletes = detail.get("athletesInvolved", [{}])
        var_events.append({
            ...
            "reason": detail.get("type", {}).get("text", "VAR Review"),
        })
    return var_events
...
def espn_get_league(slug: str, league_name: str) -> list[dict]:
    ...
    for date_str, date_prefix in [(today, today_str), (yesterday, yesterday_str)]:
        ...
        for e in data.get("events", []):
            ...
            if not n:
                continue
            # NOTE: we used to unconditionally drop any FINISHED match
            # from the "yesterday" fetch here. That's wrong for a match
            # that kicked off before midnight and finished shortly
            # after — it would get dropped the instant it finished,
            # before the bot ever got a chance to post its full-time
            # event, since it never appears in "today"'s fetch either
            # (its utcDate is still yesterday's date). Staleness is
            # handled uniformly downstream by _is_stale_finished/
            # _drop_stale (6h-since-kickoff cutoff) instead, so a
            # recently-finished midnight-crossing match still gets
            # through while a genuinely old one still gets dropped.
...
def _get_todays_matches_espn() -> list[dict]:
    """ESPN fetch — the fallback source."""
...
def get_todays_matches() -> list[dict]:
    """Public entry point. ESPN is the sole data source."""
    return _get_todays_matches_espn()
