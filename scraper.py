...
def get_lineup(league_slug: str, event_id: str) -> list[dict]:
    ...
    data = _espn_get(f"{ESPN_SUMMARY_API}/{league_slug}/summary?event={raw_id}")
    if not data:
        return []
    try:
        ...
        for i, roster in enumerate(rosters[:2]):
            ...
            if starters:
                lineups.append({
                    "team":      team_name,
                    "formation": data.get("boxscore", {}).get("formations", [{}])[i].get("displayName", "")
                                 if i < len(data.get("boxscore", {}).get("formations", [])) else "",
                    "startXI":   starters,
                })
        return lineups
    except Exception as e:
        print(f"[ESPN] Lineup parse error: {e}")
        return []
...
def get_todays_matches() -> list[dict]:
    """
    Public entry point. Sofascore is tried first (config.PRIMARY_SOURCE);
    ESPN is the automatic fallback if Sofascore returns nothing or errors.
    """
    import config

    if config.PRIMARY_SOURCE == "sofascore":
        try:
            import sofascore
            m = sofascore.get_todays_matches(_comp_flag, lambda h, a: is_national_team(h) and is_national_team(a))
            m = [
                x for x in m
                if is_international_match(x) or x.get("_comp_name") in _ALL_CLUB_NAMES
            ]
            if m:
                return _drop_stale(m)
            print("[SCRAPER] Sofascore returned 0 matches — falling back to ESPN")
        except Exception as e:
            print(f"[SCRAPER] ⚠️  Sofascore failed ({e}) — falling back to ESPN")
