...
def fmt_lineup(match: dict) -> str:
    ...
    def _match_lineup(team_name: str):
        ...
        # ESPN's roster "team" display name doesn't always match our
...
# ══════════════════════════════════════════════════════════════════
# Source headlines (ESPN/BBC/Sky/90min) are written for native
...
_NEWS_CATEGORY_STYLE = {
    ...
    "deal_collapsed":   ("DEAL COLLAPSED",        "❌", "DealCollapsed"),
...
_STORY_CONTEXT = {
    ...
    "deal_collapsed":   "That move is off in the {league} — here's what happened.",
...
def fmt_football_news(item: dict) -> str:
    """
    Confirmed football news only — deal-done and deal-collapsed
    transfers, manager sackings/appointments, injuries, and player
    quotes. No gossip, no rumours. Header, marker, and primary hashtag
    adapt to item["category"] so each flavor reads distinctly on the
    page.
    ...
    """
    category = item.get("category", "deal_done")
    header, marker, primary_tag = _NEWS_CATEGORY_STYLE.get(
        category, _NEWS_CATEGORY_STYLE["deal_done"]
    )
    ...
    if desc:
        ...
        context = _STORY_CONTEXT.get(category, _STORY_CONTEXT["deal_done"]).format(
