...
def _build_post(header: str, body: list[str], hashtags: list[str], source: str | None = None) -> str:
    lines = [f"🚩 {header}"]
    if body:
        lines.append("")
        lines.extend(body)
...
def fmt_transfer_news(item: dict) -> str:
    ...
    ⚽ [Premier League] Player X completes move to Club Y

    Source: ESPN
    #TransferNews #PremierLeague #Football
    """
    body = [f"⚽ [{item['league']}] {item['headline']}"]
...
def fmt_var_disallowed(match: dict, var_event: dict) -> str:
    ...
    return _build_post(header, body, tags)
