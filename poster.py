...
def fmt_transfer_news(item: dict) -> str:
    """
    🚨 BREAKING NEWS

    Lionel Messi completes shock move to Club X

    Source: Sky Sports
    #TransferNews #PremierLeague #Football
    """
    ...
    return _build_post(
        "BREAKING NEWS", body,
        ["TransferNews", league_tag, "Football"],
        source=item.get("source", "ESPN"),
        marker="🚨",
    )


def fmt_transfermarkt_highlight(item: dict) -> str:
    """
    🚩 Transfer Flashback:

    🔄 Player X: Club A ➜ Club B
    💰 €40.0M (or "Fee undisclosed" — never invented)
    📅 12 June 2026

    Source: transfermarkt-datasets
    #TransferWindow #Football #MatchCornaLive
    """
    fee = item.get("fee_eur")
    if fee is None:
        fee_line = "💰 Fee undisclosed"
    elif fee == 0:
        fee_line = "💰 Free transfer"
    else:
        fee_line = f"💰 €{fee / 1_000_000:.1f}M"

    date_display = item.get("transfer_date", "")

    body = [
        f"🔄 {item['player']}: {item['from_club']} ➜ {item['to_club']}",
        fee_line,
        f"📅 {date_display}",
    ]
    return _build_post(
        "Transfer Flashback", body,
        ["TransferWindow", "Football", "MatchCornaLive"],
        source="transfermarkt-datasets",
        marker="🚨",
    )
