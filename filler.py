...
COMPARISON_COOLDOWN_HOURS = 3 * 24  # a given pairing can recur every few days since the underlying numbers move
...
def _live_comparison_candidates(used: dict) -> list[tuple[str, dict]]:
    return comparisons.build_live_comparison_pool(used, COMPARISON_COOLDOWN_HOURS, _eligible)
...
def build_filler_post(used: dict) -> tuple[str, str | None] | None:
    ...
    live_comparisons = _live_comparison_candidates(used)

    candidates = [("legend", cid, entry) for cid, entry in legends] + \
                 [("live",   cid, entry) for cid, entry in live] + \
                 [("live_compare", cid, entry) for cid, entry in live_comparisons]
    if not candidates:
        return None
    ...
    if kind == "legend":
        ...
        caption = f"{hook}\n\n📊 {entry['stat_line']}"
        if entry.get("type") == "comparison" and entry.get("card_rows") and entry.get("players"):
            a_name, b_name = entry["players"][0], entry["players"][1]
            image_path = graphics.render_comparison_card("stat", a_name, b_name, entry["card_rows"])
        elif entry.get("card_stats"):
            image_path = graphics.render_stat_highlight_card("stat", entry["topic"], entry["card_stats"])
        else:
            image_path = graphics.render_headline_card("stat", entry["topic"], source=None)
        return caption, image_path

    if kind == "live_compare":
        caption = comparisons.render_live_comparison(entry)
        a, b = entry["a"], entry["b"]
        stat_rows = [
            ("GOALS",   a["goals"],   b["goals"]),
            ("ASSISTS", a["assists"], b["assists"]),
            ("APPS",    a["appearances"], b["appearances"]),
        ]
        image_path = graphics.render_comparison_card(
            "stat", a["name"], b["name"], stat_rows, a_team=a["team"], b_team=b["team"]
        )
        return caption, image_path
