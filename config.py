...
# Fires when nothing is live, at most once per FILLER_INTERVAL_MINUTES,
# following two different schedules:
#   MATCH DAY   — hourly starting at FILLER_MATCHDAY_START_HOUR (local),
#                  stopping FILLER_PRE_MATCH_STOP_HOURS before the
#                  earliest kickoff today (handing off to lineup/hype
#                  content for that final stretch). If the match finishes
#                  with time still left in the day, FILLER_RESUME_AFTER_MATCH
#                  controls whether hourly posting resumes for the rest
#                  of the FILLER_WINDOW_START_HOUR-FILLER_WINDOW_END_HOUR
#                  window.
#   NO MATCH DAY — hourly for the entire FILLER_WINDOW_START_HOUR-
...
FILLER_MATCHDAY_START_HOUR = int(os.getenv("FILLER_MATCHDAY_START_HOUR", "8"))    # 8am local
FILLER_PRE_MATCH_STOP_HOURS = float(os.getenv("FILLER_PRE_MATCH_STOP_HOURS", "2"))  # stop 2h before kickoff
FILLER_RESUME_AFTER_MATCH  = os.getenv("FILLER_RESUME_AFTER_MATCH", "true").lower() == "true"
FILLER_WINDOW_START_HOUR  = int(os.getenv("FILLER_WINDOW_START_HOUR", "6"))   # 6am local
...
# affects the windows above — everything else in the bot runs in UTC.
