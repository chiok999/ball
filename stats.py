...
# NOTE: La Liga, Ligue 1, Championship, and Champions League standings
# tables are retired while WORLD_CUP_MODE is on (config.STANDINGS_RETIRED)
# — Troy: "remove those tables ... they are making the page look weird".
# Set WORLD_CUP_MODE=false in env vars to bring them all back.
LEAGUES = [
...
_ACTIVE_LEAGUES = [
    ...
    if not (config.WORLD_CUP_MODE and lg[0] in config.STANDINGS_RETIRED)
]
...
def todays_leagues() -> tuple:
    """Returns 1-2 active (non-retired) leagues for today's standings slot(s)."""
    if not _ACTIVE_LEAGUES:
        return ()
    weekday = datetime.now(timezone.utc).weekday()
    i = weekday % len(_ACTIVE_LEAGUES)
    j = (weekday + 1) % len(_ACTIVE_LEAGUES)
    if len(_ACTIVE_LEAGUES) == 1:
        return (_ACTIVE_LEAGUES[0],)
    return (_ACTIVE_LEAGUES[i], _ACTIVE_LEAGUES[j])


# Standings slots are dropped entirely while World Cup mode is on — that
# quiet-time slot is filled by the 30-min World Cup filler system in
# bot.py instead (top scorers / win probability).
STATS_SCHEDULE = (
    {"upcoming": 19}
    if config.WORLD_CUP_MODE else
    {"standings_1": 9, "standings_2": 14, "upcoming": 19}
)

STATS_MINUTE = {k: 0 for k in STATS_SCHEDULE}
